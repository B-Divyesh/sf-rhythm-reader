...
test('a fresh install reloads the complete trainer offline without an HTML asset fallback', async ({ browser }) => {
  ...
  await page.reload();
  await expect(page.locator('h1')).toHaveText(/Don’t guess/i);
  await expect(page.locator('.tap-pad')).toBeVisible();
  await expect(page.evaluate(() => fetch('/assets/not-in-precache.js').then(() => 'response', () => 'failed'))).resolves.toBe('failed');
...
test('an old controlled client shows and explicitly activates an update', async ({ browser }) => {
  ...
  try {
    ...
    const updateButton = page.getByRole('button', { name: 'Reload update' });
